...
import dataCardDashboard from "@/components/data/json/dataCardDashboard.json";
...
const HeadTopic = ({ dataUserUpdate }: { dataUserUpdate: any }) => {
  return (
    <header className="w-full h-24 px-4 py-4 flex justify-between items-center">
...
const Topic = () => {
  ...
  return (
    <section className="w-9/12 h-full pb-4 pl-4">
      <div className=" w-full h-full flex flex-col gap-4 py-4">
        <div className="flex justify-between pr-4">
          <h2 className="capitalize text-xl ">new courses</h2>
          <p className={`capitalize text-lg text-color-c2 hover:text-blue-400 transition-all underline cursor-pointer`} onClick={handleViewAll}>{limit? "hide all" : "view all"}</p>
        </div>
        <div className="grid grid-cols-3 gap-4">
          {dataCardDashboard.slice(0, item).map((items, index) => (
            <CardNewCourses
              title={items.title}
              key={index}
        ...
        <div className="w-full h-full flex flex-col gap-6">
          ...
          <div className="w-6/12 h-full bg-white rounded-lg p-4">
            <Calendar/>
          </div>   
      </div>
      </div>
    ...
    <section className="w-3/12 h-full">
      <div className="w-full h-[232px] bg-bgCard bg-center bg-cover rounded-2xl flex p-2">
        <div className="w-1/2 h-full px-2 py-2 flex flex-col justify-between">
          <h2 className="uppercase font-semibold text-white text-sm">techfusion</h2>
          <div className="flex flex-col gap-2">
            <h4 className="text-white text-xl font-semibold capitalize">go premium</h4>
            <p className="text-sm text-white">nesciunt, delectus ea dolorem debitis</p>
          </div>
          <button onClick={() => router.push("/pre/e/premium/checkout")} className=" relative w-[150px] py-1 px-2 flex justify-center items-center capitalize rounded-full border-2 hover:bg-color-c2 transition-all border-color-c2 text-white">get access</button>
        </div>
