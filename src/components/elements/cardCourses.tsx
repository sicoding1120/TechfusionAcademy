...
const CardCourses = ({
  ...
  return (
    <div className="w-full bg-color-c10 h-full flex flex-col gap-8 md:gap-0 pb-20" key={key}>
      ...
      <div className="w-full h-full px-4">
        <div className=" w-full h-full flex flex-col md:flex-row gap-8">
          <div className="md:w-1/3 w-full h-full flex flex-col justify-center items-center md:items-start gap-3">
            <h1 className="md:text-4xl text-3xl text-color-c9 text-center md:text-start">{title}</h1>
            <p className="text-sm text-color-c9 text-center md:text-start">{subTitle}</p>
            <button className="w-[100px] flex justify-center px-4 py-2 bg-color-c9 rounded-xl">
              see all
            </button>
          </div>
